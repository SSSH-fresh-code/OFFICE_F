...
export const Route = createFileRoute('/topic/new/')({
  component: () => <TopicNewPage />
})

function TopicNewPage() {
  ...
  async function onSubmit(values: z.infer<typeof TopicSchema>) {
    if (confirm(`'${values.name}' 주제를 생성하시겠습니까?`)) {
      const topicResult = await req<ReadTopicDto>('topic', 'post', { name: values.name });

      if (topicResult.success && topicResult.data) {
        if (confirm("주제가 정상적으로 생성되었습니다.\n해당 주제로 이동하시겠습니까?")) {
    ...
    <>
      <Card className="p-5 pt-3 bg-white bg-opacity-90">
        <div className="flex justify-end">
          <Button variant="link" onClick={() => { navigate({ to: "/topic" }) }}>뒤로가기</Button>
        </div>
        <CardHeader className="pt-0">
          <CardTitle className="w-full text-center text-2xl">주제 생성</CardTitle>
        </CardHeader>
