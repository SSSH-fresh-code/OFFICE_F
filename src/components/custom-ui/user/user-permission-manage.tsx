...
function UserPermissionManage() {
  ...
  const functions = {
    save: async () => {
      ...
      if (confirm(`권한을 수정하시겠습니까?`)) {
        const userPermissionResult = await req<ReadUserDto>('user/permission', 'put', {
          id: user.id,
          permissions: perm
        });
  ...
  return (
    <div className="flex flex-col gap-2">
      <h1 className="text-center text-3xl font-bold">{PAGE_TITLE["/user/$id/permission"]}</h1>
      ...
      <div className="w-full flex justify-center">
        <div className="flex justify-between w-[480px]">
          <Button variant="secondary" onClick={functions.reset}>초기화</Button>
          <Button variant="outline" onClick={functions.save}>수정</Button>
        </div>
      </div>
    </div>
  )
