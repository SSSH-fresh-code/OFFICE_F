...
function PostDetailForm() {
	...
	useEffect(() => {
		if (topicId) {
			...
			queryClient
				.ensureQueryData(seriesQueryOptions)
				.then(({ data }) => {
	...
	const functions = {
		...
		remove: () => {
			if (confirm("해당 게시글을 삭제하시겠습니까?")) {
				req<void>(`post/${data.id}`, "delete").then((result) => {
					if (result.success) {
						alert("정상적으로 삭제되었습니다!");
						navigate({ to: "/post" });
					}
							...
							<div className="flex justify-between">
								<DropdownMenu>
									...
									<DropdownMenuContent className="bg-white">
										<SsshDropdownMenuItem onClick={functions.moveToParentTopic}>
											상위 주제로 이동
										...
										<SsshDropdownMenuItem onClick={functions.remove}>
											게시글 삭제
										</SsshDropdownMenuItem>
									</DropdownMenuContent>
								</DropdownMenu>
