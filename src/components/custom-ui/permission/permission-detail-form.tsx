...
function PermissionDetailForm() {
  ...
  async function onSubmit(values: z.infer<typeof PermissionSchema>) {
    ...
    if (confirm(`'권한 설명을 변경하시겠습니까?`)) {
      const PermissionResult = await req<ReadPermissionDto>('permission', 'put', values);
    ...
    <>
      <Card className="p-5 pt-3 bg-white bg-opacity-90">
        <div className="flex justify-end">
          <Button variant="link" onClick={() => { navigate({ to: "/permission" }) }}>목록</Button>
        </div>
        <CardHeader className="pt-0">
          <CardTitle className="w-full text-center text-2xl">권한 상세정보</CardTitle>
        </CardHeader>
